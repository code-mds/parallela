...
public class S4Esercizio1 {
	...
	public static long getValueRWExp() {
		readlock.lock();
		try {
			try {
				Thread.sleep(1);
	...
	private static long incrementExp() {
		lock.lock();
		try {
			value++;
			try {
	...
	public static long getValueExp() {
		lock.lock();
		try {
			try {
				Thread.sleep(1);
	...
	private static synchronized long incrementImplicit() {
		value++;
		try {
	...
	private static synchronized long getValueImplicit() {
		try {
